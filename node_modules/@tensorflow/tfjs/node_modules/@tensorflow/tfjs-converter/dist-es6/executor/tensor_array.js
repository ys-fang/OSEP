...
var TensorArray = (function () {
    ...
    TensorArray.prototype.write = function (index, tensor) {
        ...
        util.assertShapesMatch(this.elementShape, tensor.shape, "TensorArray " + this.name + ": Could not write to TensorArray index " + index + ".");
        if (t && t.read) {
    ...
    TensorArray.prototype.gather = function (indices, dtype) {
        ...
        util.assertShapesMatch(this.elementShape, tensors[0].shape, 'TensorArray shape mismatch: ');
    ...
    TensorArray.prototype.concat = function (dtype) {
        ...
        var tensors = this.readMany(indices);
        util.assertShapesMatch(this.elementShape, tensors[0].shape, "TensorArray shape mismatch: tensor array shape (" + this.elementShape + ") vs first tensor shape (" + tensors[0].shape + ")");
    ...
    TensorArray.prototype.split = function (length, tensor) {
        ...
        tidy(function () {
            ...
            for (var i = 0; i < length.length; ++i) {
                ...
                tensors[i] = slice(tensor, indices_1, sizes).reshape(_this.elementShape);
            }
            return tensors;
        });
        var indices = [];
