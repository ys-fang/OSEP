...
var LinalgOps = (function () {
    ...
    LinalgOps.gramSchmidt = function (xs) {
        ...
        if (Array.isArray(xs)) {
            ...
            xs =
                array_ops_1.ArrayOps.split(xs, xs.shape[0], 0).map(function (x) { return array_ops_1.ArrayOps.squeeze(x, [0]); });
        }
        ...
        var _loop_1 = function (i) {
            ys.push(environment_1.ENV.engine.tidy(function () {
                ...
                if (i > 0) {
                    for (var j = 0; j < i; ++j) {
                        var proj = reduction_ops_1.ReductionOps.sum(ys[j].mulStrict(x)).mul(ys[j]);
                        x = x.sub(proj);
                    }
                }
                return x.div(norm_1.NormOps.norm(x, 'euclidean'));
            }));
        };
        ...
        if (inputIsTensor2D) {
            return array_ops_1.ArrayOps.stack(ys, 0);
        }
        else {
            return ys;
        }
    ...
    LinalgOps.qr = function (x, fullMatrices) {
        ...
        if (x.rank < 2) {
            ...
            var x2ds = array_ops_1.ArrayOps.unstack(x.reshape([
            ...
            var q = array_ops_1.ArrayOps.stack(q2ds_1, 0).reshape(x.shape);
            var r = array_ops_1.ArrayOps.stack(r2ds_1, 0).reshape(x.shape);
            return [q, r];
        }
    };
    __decorate([
...
function qr2d(x, fullMatrices) {
    ...
    return environment_1.ENV.engine.tidy(function () {
        ...
        var m = x.shape[0];
        var n = x.shape[1];
        var q = array_ops_1.ArrayOps.eye(m);
        var r = x.clone();
        var one2D = tensor_ops_1.TensorOps.tensor2d([[1]], [1, 1]);
        var w = one2D.clone();
        var iters = m >= n ? n : m;
