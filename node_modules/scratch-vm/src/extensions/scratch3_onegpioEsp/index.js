...
const FormDigitalWrite = {
    ...
    'zh-tw': 'GPIO[PIN]數位輸出[ON_OFF]',
    'zh-cn': 'GPIO[PIN]数位输出[ON_OFF]',
...
const FormPwmWrite = {
    ...
    'zh-tw': 'GPIO[PIN]類比輸出[VALUE]%',
    'zh-cn': 'GPIO[PIN]类比输出[VALUE]%',
...
const FormTone = {
    ...
    'zh-tw': 'GPIO[PIN]播放音調，頻率為[FREQ]時間為[DURATION]',
    'zh-cn': 'GPIO[PIN]播放音调，频率为[FREQ]时间为[DURATION]',
...
const FormServo = {
    ...
    'zh-tw': '伺服馬達GPIO[PIN]轉動角度到[ANGLE]度',
    'zh-cn': '伺服马达GPIO[PIN]转动角度到[ANGLE]度',
...
const FormSonarRead = {
    ...
    'zh-tw': 'HCSR超音波感測器，Echo在GPIO[ECHO_PIN]Trig在GPIO[TRIGGER_PIN]',
    'zh-cn': 'HCSR超音波感测器，Echo在GPIO[ECHO_PIN]Trig在GPIO[TRIGGER_PIN]',
...
class Scratch3EspOneGPIO {
    ...
    digital_write(args) {
        ...
        if (!connected) {
            ...
            const pin_array = args['PIN'].split('(');
	    let pin = pin_array[0];
            pin = parseInt(pin, 10);
