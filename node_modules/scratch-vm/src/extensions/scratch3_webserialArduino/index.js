...
class Scratch3ArduinoWebSerial {
    ...
    async oled_show(args){
        ...
        console.log('sendData=',sendData);
        const encoder = new TextEncoder();
        const oled_writer = port.writable.getWriter();
        await oled_writer.write(encoder.encode(sendData));
        oled_writer.releaseLock();
    }
    ...
    async digitalWrite(args){
        ...
        //value = parseInt(value, 10);
        let sendData;
        sendData = 'digitalWrite#'+ pin.toString()+'#'+value.toString();
        const encoder = new TextEncoder();
        const writer = port.writable.getWriter();
        await writer.write(encoder.encode(sendData));
        writer.releaseLock();
    ...
    async dht11_read(args){
        ...
        let sendData = 'dht11Read#'+pin.toString()+'#'+send_th;
        console.log('sendData:',sendData);
        const encoder = new TextEncoder();
        let writer = port.writable.getWriter();
        ...
        let string = new TextDecoder().decode(value.value);
        string = string.split('\r\n');
        const dht11_inputs = string[0];
        //console.log('digital_inputs[pin]=',digital_inputs);
        reader.releaseLock();
        //console.log('reader release:',port.locked);
        return dht11_inputs;
    }
