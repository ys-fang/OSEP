...
class Scratch3PicoboardWebSerial {
    ...
    async getSensorValue(){
        ...
        while(isread){   
            ...
            let Picoboard_writer = Picoboard_port.writable.getWriter();
            await Picoboard_writer.write(encoder.encode(sendData));
            Picoboard_writer.releaseLock();
         
            let reader = Picoboard_port.readable.getReader();
            let avalue = await reader.read();
            //console.log('avalue',avalue);
            let uint8array = new TextEncoder().encode();
            let string = new TextDecoder().decode(avalue.value);
            string = string.split('\r\n');
                serial_request = string[0];
                console.log(serial_request);
                serial_request = serial_request.split(',');
                for (let i=0; i<serial_request.length; i++) {
                serial_request[i]= parseInt(serial_request[i],10);
                }
                //console.log('serial_request length=',serial_request.length);          
                if (serial_request.length == 8){
                    isread = false;
                }
            reader.releaseLock();
            
            }    
            lastDataSample = serial_request;
            //return lastDataSample ;  
    }
    ...
    async range_convert(args) {
            ...
            return Math.round(((value-512)/512 * 240));
            //return Math.round(((value-512)/512 * ((high - low) / 100)) +low);
    }
