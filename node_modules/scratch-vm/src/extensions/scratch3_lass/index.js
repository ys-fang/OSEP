        ...
        return fetch(url).then(res => {
            if (res.ok) {
			   ...
			   for (const [key, value] of Object.entries(bdata)) {                    
                ...
                var returndata =[];
                for (x in this.aqidata) {
			returndata.push(this.aqidata[x]) ;
		};
                ...
                if (attr =='PM2.5'){
                	console.log('PM2.5');
                	return returndata[11];
                	//return typeof data === 'string' ? data : JSON.stringify(this.aqidata[11]');
                }
